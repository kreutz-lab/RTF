...
#' log-likelihood value is not significant for Chi-square significance test.
#' @return optimObject of the smaller model if for the bigger and smaller model
#' there is no significant difference in the log-likelihood values achieved
#' through fitting.
#' An optimObject is a list containing input data frame with time-resolved data
#' ('data'), the vector of initial guesses ('initialGuess.vec'), of lower
...
#' ('positive.par.names'), modus ('modus'), log-likelihood function for the
#' parameter optimization ('optimFunction'), if the sign of the sustained and
#' transient RTF part should be equal ('sameSign'), list of control parameters
#' passed to stats::optim ('control'), and a list of values of fitted
#' parameters ('fitted', can be empty).
