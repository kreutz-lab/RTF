...
#' @param waterfallValues List of log-likelihood values or, if idxCurrentFit is
#'  given, data frame with columns indicating for each fit "value"
#'  (log-likelihood value), "idx" (position of a fit in the sequence
...
plotWaterfallPlot <- function(waterfallValues, idxCurrentFit = NULL,
    ...
    gg <- ggplot2::ggplot(
        ...
        ggplot2::labs(x = "Index", y = "Log-likelihood") +
        ggplot2::scale_colour_manual(values = c("#000000", "#FF0000")) +
        ggplot2::theme_bw() +
        ggplot2::theme(legend.position = "none") +
        ggplot2::xlim(NA, max(which(df$value <= maxValue))) +
