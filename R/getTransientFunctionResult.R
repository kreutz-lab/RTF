...
#' y <- getTransientFunctionResult(rtfPar = rtfPar, t = t)
#' plot(t, y)
...
getTransientFunctionResult <- function(rtfPar = c(),
  ...
  rtfParamNames <- c("alpha", "gamma", "A", "B", "tau", "b", "signum_TF")
  
  rtfPar <- rtfPar[rtfParamNames]
  
  if (length(rtfPar) < length(rtfParamNames)) {
    stop("Parameters alpha, gamma, A, B, tau, b, signum_TF need to be provided for getTransientFunctionResult().")
  }
  ...
  if (calcGradient) {
    ...
    dtransFunRes_dparScaledRtf[,"alpha"] <- A * signum_TF * nonLinTransformation * 
    ...
    dtransFunRes_dparRtf <- dtransFunRes_dparScaledRtf %*% dparScaled_dpar + 
      dtransFunRes_dnonLinTrans %*% dnonLinTrans_dparScaledRtf %*%dparScaled_dpar 
